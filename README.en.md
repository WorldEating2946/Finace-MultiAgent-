...
One-click end-to-end research pipeline: **Manager intent planning → parallel (Research RAG ∥ Financial hard metrics ∥ Sentiment) → health-check retry loop → Risk assessment → Report assembly → quality iteration → report output** (Markdown / HTML dual export + live SSE node progress).

The screenshot below shows a real run that generated a full report for **BYD (002594)** (multi-agent end-to-end view):

- **Flow visualization**: the 5-agent pipeline (Manager → 3 parallel paths → Risk → Report); the SSE node tape surfaces each stage (Manager dispatch → fan-out → sentiment / financial / fundamentals → health check → risk → drafting → evaluation → revision, **10 successful nodes** this run).
- **Structured output**: the complete six-chapter report renders below, exportable as Markdown / HTML.

![Full-chain report generation · BYD 002594](docs/screenshots/fullchain-report.png)

**Latest sample report** (generated 2026-08-25T15:59:22 · path `app/data/reports/report_20260825_155922/report.md`):

| Chapter | Core content | Source |
|---|---|---|
| 1. Company overview | 002594 · BYD | RAG knowledge base |
| 2. Financial analysis | ROE 13.2% · net margin 4.1% · equity multiplier 3.59 · asset turnover 0.9097 · DuPont · growth/operations/leverage breakdown | Structured API (Pandas) |
| 3. Industry & competitiveness | — (pending) | RAG |
| 4. Sentiment | 10 news items · bullish 9 / bearish 1 / neutral 0 | News + Chinese sentiment |
| 5. Risk | Overall **LOW** (score 0.060 / 1.0) · 3-dimension reasoning (sentiment 0.15 / financial 0.0 / industry 0.0) | Rules + reasoning |
| 6. Investment view | Low risk; suitable for deep study + disclaimer | Full pipeline |
